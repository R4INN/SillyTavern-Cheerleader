// SillyTavern Cheerleader Extension v3.0.0 - Unified Feature Update
// Author: Antigravity
// v3 Features: Typewriter animation, confetti effects, mood detection, streak system,
//              quick reactions, statistics, slash commands, sound notifications,
//              toast notifications, markdown rendering, auto-retry, history search
//              & favorites, copy/regen buttons, 11 CSS themes, touch support
  ...
  },
  {
    ...
    function getDefaultSettings() {
        return {
            ...
            soundType: DEFAULTS.SOUND_TYPE,
            markdownEnabled: DEFAULTS.MARKDOWN_ENABLED,
    ...
    function getSettings() {
        ...
        if (settings.soundEnabled === undefined) settings.soundEnabled = DEFAULTS.SOUND_ENABLED;
        if (settings.soundType === undefined) settings.soundType = DEFAULTS.SOUND_TYPE;
        if (settings.markdownEnabled === undefined) settings.markdownEnabled = DEFAULTS.MARKDOWN_ENABLED;
    ...
    function showToastHype(text, charName, avatarHtml, mood) {
        const settings = getSettings();
        const moodData = mood ? getMoodData(mood) : null;
        const displayText = settings.markdownEnabled ? renderSimpleMarkdown(text) : $('<span>').text(text).html();

        const $toast = $(`
            <div class="cheerleader-toast">
                <div class="cheerleader-toast-header">
                    ${avatarHtml}<span class="cheerleader-toast-name">${charName}</span>
                    ${moodData ? `<span class="cheerleader-mood-badge" style="background:${moodData.color};font-size:0.65em;padding:1px 6px">${moodData.emoji}</span>` : ''}
                    <span class="cheerleader-toast-close">✕</span>
                </div>
                <div class="cheerleader-toast-content">${displayText}</div>
            </div>
        `);
    ...
    function showHypeOutput(text, mood) {
        ...
        if (position === 'toast') {
            showToastHype(text, charName, avatarHtml, mood);
            addToChatHistory(text, mood);
            State.messagesSinceLastHype = 0;
            if (settings.soundEnabled) playNotificationSound(settings.soundType || 'chime');
            return;
        }
        ...
        } else if (settings.markdownEnabled) {
    ...
    async function generateHype(retryCount = 0) {
        ...
        try {
            ...
            if (settings.retryOnFailure && retryCount < maxRetries) {
                log(`Retrying generation (attempt ${retryCount + 1}/${maxRetries})...`);
                toastr.info(`Retrying... (${retryCount + 1}/${maxRetries})`);
                State.isGenerating = false;
                setTimeout(() => generateHype(retryCount + 1), 1500);
                return;
            }
    ...
    function updateUI() {
        ...
        $('#cheerleader-markdown-enabled').prop('checked', s.markdownEnabled !== false);
        $('#cheerleader-retry-enabled').prop('checked', s.retryOnFailure !== false);
    ...
    function bindEvents() {
        ...
        $doc.on('change', '#cheerleader-markdown-enabled', function () {
            getSettings().markdownEnabled = this.checked;
            saveSettings();
        });

        $doc.on('change', '#cheerleader-retry-enabled', function () {
    ...
    function showHistoryPopup() {
        ...
        function renderHistoryItems(filter = '', favsOnly = false) {
            ...
            return history.map((entry, i) => {
                ...
                if (favsOnly && !fav) return '';
                if (lowerFilter && !msg.toLowerCase().includes(lowerFilter) && !persona.toLowerCase().includes(lowerFilter)) return '';
                ...
                return `<div class="cheerleader-history-item${fav ? ' cheerleader-history-fav' : ''}" data-index="${i}">
                    <div class="cheerleader-history-meta">
                        ...
                        <span class="cheerleader-history-star" data-index="${i}" title="Toggle favorite">${fav ? '★' : '☆'}</span>
                    </div>
        ...
        $('body').append(`
            ...
            <div id="cheerleader-history-popup" class="cheerleader-popup">
                ...
                <div class="cheerleader-history-toolbar">
                    <input id="cheerleader-history-search" class="text_pole" type="text" placeholder="Search messages..." style="flex:1">
                    <label class="checkbox_label" style="white-space:nowrap;font-size:0.85em">
                        <input type="checkbox" id="cheerleader-history-favs-only"> ★ only
                    </label>
                    <button id="cheerleader-export-history" class="menu_button" title="Export History"><i class="fa-solid fa-download"></i></button>
                </div>
                <div class="cheerleader-history-list">${renderHistoryItems()}</div>
            </div>
        `);

        function refreshList() {
            const filter = $('#cheerleader-history-search').val() || '';
            const favsOnly = $('#cheerleader-history-favs-only').prop('checked');
            $('.cheerleader-history-list').html(renderHistoryItems(filter, favsOnly));
        }

        $(document).on('input.cheerHist', '#cheerleader-history-search', refreshList);
        $(document).on('change.cheerHist', '#cheerleader-history-favs-only', refreshList);

        $(document).on('click.cheerHist', '.cheerleader-history-star', function () {
            const idx = parseInt($(this).data('index'));
            toggleFavorite(chatId, idx);
            refreshList();
        });

        $(document).on('click.cheerHist', '#cheerleader-export-history', () => {
                            ...
                            <div class="inline-drawer-content">
                                ...
                                <div class="cheerleader-section">
                                    <label class="checkbox_label">
                                        ...
                                        Sound Notification
                                        <span class="cheerleader-tooltip" title="Play a sound when hype is generated">?</span>
                                    </label>
                                    <div class="cheerleader-section cheerleader-indent">
                                ...
                                <div class="cheerleader-section">
                                    <label class="checkbox_label">
                                        <input type="checkbox" id="cheerleader-markdown-enabled">
                                        Markdown Rendering
                                        <span class="cheerleader-tooltip" title="Render **bold**, *italic*, ~~strikethrough~~, and \`code\` in hype text">?</span>
                                    </label>
                                </div>

                                <div class="cheerleader-section">
                                    <label class="checkbox_label">
                                        <input type="checkbox" id="cheerleader-retry-enabled">
                                        Auto-Retry on Failure
                                        <span class="cheerleader-tooltip" title="Automatically retry once if generation fails">?</span>
                                    </label>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="inline-drawer-content">
                                <div class="cheerleader-section">
                                    ...
                                    <select id="cheerleader-output-position" class="text_pole">
                                        ...
                                        <option value="toast">Toast Notification (Corner)</option>
                                    </select>
                                </div>
    ...
    <style>
        ...
        /* Header Actions (Copy/Regen) */
        .cheerleader-header-actions { display: flex; align-items: center; gap: 6px; }
        .cheerleader-action-btn { cursor: pointer; opacity: 0.5; transition: opacity 0.2s, transform 0.15s; padding: 2px 4px; font-size: 0.85em; }
        .cheerleader-action-btn:hover { opacity: 1; transform: scale(1.15); }

        /* Toast Notifications */
        .cheerleader-toast {
            position: fixed; bottom: 20px; right: 20px; z-index: 10000;
            background: var(--SmartThemeBlurTintColor); border: 1px solid var(--SmartThemeBorderColor);
            border-radius: 12px; padding: 12px 16px; max-width: 350px; min-width: 250px;
            backdrop-filter: blur(10px); box-shadow: 0 8px 32px rgba(0,0,0,0.3);
            animation: cheerleader-toast-enter 0.3s ease-out; transition: transform 0.3s, opacity 0.3s;
        }
        .cheerleader-toast-exit { animation: cheerleader-toast-leave 0.3s ease-in forwards; }
        .cheerleader-toast-header { display: flex; align-items: center; gap: 6px; margin-bottom: 6px; font-weight: bold; color: gold; font-size: 0.9em; }
        .cheerleader-toast-name { flex: 1; }
        .cheerleader-toast-close { cursor: pointer; opacity: 0.6; transition: opacity 0.2s; }
        .cheerleader-toast-close:hover { opacity: 1; }
        .cheerleader-toast-content { font-style: italic; font-size: 0.9em; line-height: 1.5; }
        @keyframes cheerleader-toast-enter { from { transform: translateX(100%); opacity: 0; } to { transform: translateX(0); opacity: 1; } }
        @keyframes cheerleader-toast-leave { from { transform: translateX(0); opacity: 1; } to { transform: translateX(100%); opacity: 0; } }

        /* History Toolbar */
        .cheerleader-history-toolbar { display: flex; gap: 6px; align-items: center; margin-bottom: 12px; padding-bottom: 8px; border-bottom: 1px solid var(--SmartThemeBorderColor); }
        .cheerleader-history-star { cursor: pointer; font-size: 1.1em; transition: transform 0.15s; }
        .cheerleader-history-star:hover { transform: scale(1.3); }
        .cheerleader-history-fav { border-left: 3px solid gold; }

        /* Markdown in output */
        #cheerleader-output-content strong { font-weight: 700; }
        #cheerleader-output-content em { font-style: italic; }
        #cheerleader-output-content del { text-decoration: line-through; opacity: 0.7; }
        #cheerleader-output-content code { background: rgba(0,0,0,0.3); padding: 1px 4px; border-radius: 3px; font-family: monospace; font-size: 0.9em; }
    </style>
    `;
    ...
    function init() {
        ...
        log('Initialized v3.0.0 (unified)');
    }
